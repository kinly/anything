...
static constexpr inline clock::time_point current_time_point(
    long long offset = 0) noexcept {
...
static constexpr inline timestamp relative_timestamp(
    std::chrono::duration<_Rep, _Period> offset) noexcept {
  return std::chrono::duration_cast<_Duration>(
...
template <typename _Duration = default_duration, typename _Rep = long long,
          typename _Period = std::milli>
static constexpr inline timestamp relative_days_timestamp(
    std::chrono::duration<_Rep, _Period> offset) noexcept {
  return std::chrono::duration_cast<_Duration>(
             std::chrono::duration_cast<days>(
                 current_time_point(offset).time_since_epoch()))
...
template <typename _Duration = default_duration, typename _Rep = long long,
          typename _Period = std::milli>
static constexpr inline timestamp relative_hours_timestamp(
    std::chrono::duration<_Rep, _Period> offset) noexcept {
  return std::chrono::duration_cast<_Duration>(
             std::chrono::duration_cast<hours>(
                 current_time_point(offset).time_since_epoch()))
...
/// @tparam future_point_vv fixed to future
...
/// future_point_vv == true.
/// eg. current="2024-08-08 10:00:00", source="08:00:00": result="2024-08-09 08:00:00"
/// )
template <bool future_point_vv = false>
static inline constexpr std::pair<clock::time_point, bool>
from_local_time_string(const char* source) {
  return __inner_from_time_string_v(std::string_view(source), future_point_vv,
                                    localtime_exchanger);
}

//////////////////////////////////////////////////////////////////////////
/// timespan
template <typename ret_duration_tt>
static constexpr ret_duration_tt timespan(clock::time_point left,
                                        clock::time_point right) {
  return std::chrono::duration_cast<ret_duration_tt>(left - right);
}

template <typename ret_duration_tt>
static constexpr ret_duration_tt timespan(timestamp left, timestamp right) {
  return timespan<ret_duration_tt>(from_timestamp(left), from_timestamp(right));
}

template <typename ret_duration_tt = default_duration>
static constexpr long long timespan_count(clock::time_point left,
                                          clock::time_point right) {
  return timespan<ret_duration_tt>(left - right).count();
}

template <typename ret_duration_tt>
static constexpr long long timespan_count(timestamp left, timestamp right) {
  return timespan_count<ret_duration_tt>(from_timestamp(left),
